---
title: "Data Science With R - part 03"
...
```{r}
df_selected <- readRDS("datasets/intermediary/df_selected.rds" )
...
saveRDS(lr_tune, "model/lr_tune.rds")
```

```{r}
lr_tune <- readRDS("model/lr_tune.rds")
...
saveRDS(lr_res, "model/lr_res.rds")
```

```{r}
lr_res <- readRDS("model/lr_res.rds")
...
print(end_time - start_time )
saveRDS(tree_tune, "model/tree_tune.rds")
```

```{r}
tree_tune <- readRDS("model/tree_tune.rds")
...
saveRDS(tree_res, "model/tree_res.rds")
```

```{r}
# Confusion matrix
tree_res <- readRDS("model/tree_res.rds")
...
print(end_time - start_time )
saveRDS(rf_tune, "model/rf_tune.rds")
```
...
saveRDS(rf_res, "model/rf_res.rds")
```

```{r}
# Confusion matrix
rf_res <- readRDS("model/rf_res.rds")
...
print(end_time - start_time )
saveRDS(xgb_tune, "model/xgb_tune.rds")
```

```{r}
xgb_tune <- readRDS("model/xgb_tune.rds")
...
saveRDS(xgb_res, "model/xgb_res.rds")
```

```{r}
# Confusion matrix -----------------
xgb_res <- readRDS("model/xgb_res.rds")
...
print(end_time - start_time )
saveRDS(knn_tune, "model/knn_tune.rds")
```
...
```{r}
# Confusion matrix -----------------
knn_res <- readRDS("model/knn_res.rds")
...
saveRDS(final_model, "model/final_model.rds")
```
